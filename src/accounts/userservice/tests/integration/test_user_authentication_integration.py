...
class TestUserAuthentication:
    ...
    def test_login_success_with_valid_credentials(self, test_app, sample_user_data, test_keys):
        ...
        login_response = test_app.get(f'/login?username={sample_user_data["username"]}&password={sample_user_data["password"]}')
    ...
    def test_multiple_logins_same_user(self, test_app, sample_user_data, test_keys):
        ...
        import time
        login_response1 = test_app.get(f'/login?username={sample_user_data["username"]}&password={sample_user_data["password"]}')
        time.sleep(1)  # Wait 1 second to ensure different timestamps
        login_response2 = test_app.get(f'/login?username={sample_user_data["username"]}&password={sample_user_data["password"]}')
